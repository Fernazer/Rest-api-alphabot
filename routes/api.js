...
const listkey = ["Fernazer", "fernazer"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Fernazer') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Fernazer')  return res.json(loghandler.invalidKey)
...
router.get('/jadwalshalat', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Fernazert') return res.json(loghandler.invalidKey)
